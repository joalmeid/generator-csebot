...
function findOrCreateProject(gen, callback) {
   ...
   util.tryFindProject(gen.tfs, gen.botName, token, gen, function (error, obj) {
      if (error) {
         callback(error, null);
      } else {
         ...
         gen.log(`+ Creating ${gen.botName} Team Project`);
         ...
         async.series([
            function (thisSeries) {
               createProject(gen.tfs, gen.botName, token, gen, function (err, project) {
                  teamProject = project;
                  thisSeries(err);
               });
            },
            ...
            function (thisSeries) {
               var options = util.addUserAgent({
                  ...
                  url: `${util.getFullURL(gen.tfs)}/_apis/projects/${gen.botName}`,
