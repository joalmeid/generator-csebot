...
You now have full CI/CD pipelines, that will build and deploy your bot to the cloud. Take a look at your project [bottsdevv4](https://joalmeid.visualstudio.com/bottsdevv4).
Start by pushing the starting code, and doing a first build (*will start automatically*):

    git push

In the [build](https://joalmeid.visualstudio.com/bottsdevv4/_build), you have the **bottsdevv4-CI** build definition that will install all npm modules, run tsc, run tests and publish build artifacts.
In the [Release](https://joalmeid.visualstudio.com/bottsdevv4/_release), you have **bottsdevv4-CD** defining a pipeline with Dev, QA and Prod environments. For each environment the release process consists in Azure Resource Group deployment (check the ARM templates) and your bot app deployment.
