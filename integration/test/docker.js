...
function runTests(iteration) {
   ...
   // Arguments
   iteration.botName = iteration.appName + iteration.uuid.substring(0, 8);
   ...
   context(`${iteration.appType}`, function () {
      ...
      context(`Execute command line`, function () {
         it(`Should complete without error`, function (done) {
            ...
            let cmd = `yo team ${iteration.appType} ${iteration.botName} ${tfs} ${azureSub} "${azureSubId}" ` +
      ...
      context(`Push code to remote`, function () {
         it(`git push should succeed`, function (done) {
            util.log(`cd to: ${__dirname}${levelsUp}${iteration.botName}`);
            process.chdir(`${__dirname}${levelsUp}${iteration.botName}`);
